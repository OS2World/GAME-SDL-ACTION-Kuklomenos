/* 
 ...
 License changed from LGPL to GPLv3, as per section 3 of the LGPL,
     by M. Bays 2008.
 Dirty pixels/rects handling added by M. Bays 2008.
...
SDL_Surface* dirtyDst = NULL;
SDL_Surface* dirtyBackground = NULL;
std::vector<Uint8*> dirtyPixels;
std::vector<SDL_Rect> dirtyRects;

// Set surface to accumulate dirtiness information for subsequent calls to
// blankDirty, which will then redraw the background over dirtied pixels.
// &background should be either a surface of the same size and format as &dst,
// or NULL to indicate a black background.
void setDirty(SDL_Surface* dst, SDL_Surface* background)
{
    dirtyDst = dst;
...
int blankDirty()
{
     ...
     * Lock the surface 
     */
    if (SDL_MUSTLOCK(dirtyDst)) {
	if (SDL_LockSurface(dirtyDst) < 0) {
	    return (-1);
	}
    }
    ...
    while (!dirtyPixels.empty())
    {
	Uint8* p = dirtyPixels.back();
	dirtyPixels.pop_back();
	int bpp = dirtyDst->format->BytesPerPixel;
	if (dirtyBackground)
	{
	    const Uint8* bp = (Uint8*)dirtyBackground->pixels + (p -
		    (Uint8*)dirtyDst->pixels);
	    for (int i=0; i<bpp; i++)
		*(p+i) = *(bp+i);
	}
	else
     ...
     */
    if (SDL_MUSTLOCK(dirtyDst)) {
	SDL_UnlockSurface(dirtyDst);
    }

    while (!dirtyRects.empty())
    {
	SDL_Rect rect = dirtyRects.back();
	dirtyRects.pop_back();
	if (dirtyBackground)
	    SDL_BlitSurface(dirtyBackground, &rect, dirtyDst, &rect);
	else
	    SDL_FillRect(dirtyDst, &rect, 0);
    }
...
int _filledRectAlpha(SDL_Surface * surface, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint32 color, Uint8 alpha)
{
    ...
    case 2:{			/* Probably 15-bpp or 16-bpp */
		...
		for (x = x1; x <= x2; x++) {
		    ...
		    *pixel = R | G | B | A;
		    if (dirtyDst == surface)
			dirtyPixels.push_back( (Uint8*) pixel );
		}
	    }
	}
	break;
    ...
    case 4:{			/* Probably :-) 32-bpp */
	    ...
	    Uint32 dR = (color & Rmask), dG = (color & Gmask), dB = (color & Bmask), dA = (color & Amask);
            Uint32 dc;
            Uint32 preMultR, preMultG, preMultB, preMultA;
            Uint32 aTmp;
		...
		for (x = x1; x <= x2; x++) {
		    ...
		    dc = *pixel;
                    ...
                    aTmp = (255 - alpha);
		    R = (preMultR + (aTmp * ((dc & Rmask) >> Rshift))) >> 8 << Rshift & Rmask;
		    G = (preMultG + (aTmp * ((dc & Gmask) >> Gshift))) >> 8 << Gshift & Gmask;
		    B = (preMultB + (aTmp * ((dc & Bmask) >> Bshift))) >> 8 << Bshift & Bmask;
		    if (Amask)
			A = (preMultA + (aTmp * ((dc & Amask) >> Ashift))) >> 8 << Ashift & Amask;
		    *pixel = R | G | B | A;
		    if (dirtyDst == surface)
			dirtyPixels.push_back(pixel);
		}
...
int hlineColorStore(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, Uint32 color)
{
	...
	pixellast = pixel + dx * pixx;
	for (Uint8* tpixel = pixel; tpixel <= pixellast; tpixel += pixx)
	    if (dirtyDst == dst)
		dirtyPixels.push_back(tpixel);
...
int hlineColor(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, Uint32 color)
{
	...
	pixy = dst->pitch;
	pixel = ((Uint8 *) dst->pixels) + pixx * (int) x1 + pixy * (int) y;
	pixellast = pixel + dx * dst->format->BytesPerPixel;
	for (Uint8* tpixel = pixel; tpixel <= pixellast; tpixel += pixx)
	    if (dirtyDst == dst)
		dirtyPixels.push_back(tpixel);
...
int vlineColor(SDL_Surface * dst, Sint16 x, Sint16 y1, Sint16 y2, Uint32 color)
{
	...
	for (Uint8* tpixel = pixel; tpixel <= pixellast; tpixel += pixy)
	    if (dirtyDst == dst)
		dirtyPixels.push_back(tpixel);
...
double antialiasGamma = 2.2;
WeightPair getWeights(Uint8 balance)
{
    // Gamma-correction for anti-aliasing calculations - get weights for a
    // pair of pixels.
    //
    // We take gamma to be 2.2 by default, which seems generally to be about
    // right (the exact value being system-dependent...)

    static WeightPair precalced[256];
    static double precalcedGamma = -1;

    if (precalcedGamma != antialiasGamma)
    {
	for (Uint16 b = 0; b < 256; b++)
	{
	    precalced[b].main =
		(Uint8) (255.0*pow(b/255.0, 1.0/antialiasGamma));
	    precalced[b].complement =
		(Uint8) (255.0*pow((255-b)/255.0, 1.0/antialiasGamma));
	}
	precalcedGamma = antialiasGamma;
    }

    return precalced[balance];
}
...
int aalineColorInt(SDL_Surface * dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint32 color, int draw_endpoint)
{
	...
	while (--dy) {
	     ...
	     * the paired pixel. 
	     */
	    balance = (erracc >> intshift) & 255;
	    weightPair = getWeights(balance);

	    result |= pixelColorWeightNolock (dst, xx0, yy0, color,
		    weightPair.complement);
	    result |= pixelColorWeightNolock (dst, x0pxdir, yy0, color,
		    weightPair.main);
	}
	...
	while (--dx) {
	    ...
	    result |= pixelColorWeightNolock (dst, xx0, yy0, color,
		    weightPair.complement);
	    result |= pixelColorWeightNolock (dst, xx0, y0p1, color,
		    weightPair.main);
	}
    }
...
int arcColor(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 r, Sint16 start, Sint16 end, Uint32 color)
{
	...
	int stopval_start=0, stopval_end=0; // what values of cx to stop at.
	double temp=0;
...
int aaellipseColor(SDL_Surface * dst, Sint16 xc, Sint16 yc, Sint16 rx, Sint16 ry, Uint32 color)
{
    ...
    /*
     * Special case for rx=1=ry - draw non-aa version
     * (this is to work around a bug in the aa code below)
     */
    if (rx == 1 && ry == 1)
	return ellipseColor(dst, xc, yc, rx, ry, color);
	...
	/* Calculate weights */
	balance = (Uint16) (cp * 255);
	weightPair = getWeights(balance);

	/* Upper half */
	xx = xc2 - x;
	result |= pixelColorWeightNolock(dst, x, y, color, weightPair.complement);
	result |= pixelColorWeightNolock(dst, xx, y, color, weightPair.complement);

	result |= pixelColorWeightNolock(dst, x, ys, color, weightPair.main);
	result |= pixelColorWeightNolock(dst, xx, ys, color, weightPair.main);

	/* Lower half */
	yy = yc2 - y;
	result |= pixelColorWeightNolock(dst, x, yy, color, weightPair.complement);
	result |= pixelColorWeightNolock(dst, xx, yy, color, weightPair.complement);

	yy = yc2 - ys;
	result |= pixelColorWeightNolock(dst, x, yy, color, weightPair.main);
	result |= pixelColorWeightNolock(dst, xx, yy, color, weightPair.main);
    }
	...
	/* Right half */
	xx = 2 * xc - xs;
	result |= pixelColorWeightNolock(dst, xs, y, color, weightPair.main);
	result |= pixelColorWeightNolock(dst, xx, y, color, weightPair.main);

	result |= pixelColorWeightNolock(dst, xs, yy, color, weightPair.main);
	result |= pixelColorWeightNolock(dst, xx, yy, color, weightPair.main);
...
int _texturedHLine(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y,SDL_Surface *texture,int texture_dx,int texture_dy)
{
    ...
    if (w <= texture->w -texture_x_walker){
      ...
      dst_rect.x= x1;
      if (dirtyDst == dst)
	  dirtyRects.push_back(dst_rect);
      SDL_BlitSurface  (texture,&source_rect , dst, &dst_rect) ;
